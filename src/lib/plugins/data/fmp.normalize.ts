...
export function normalizeRatios(raw: FmpRatio[]): NormalizedHistoricalRatio[] {
  return (raw ?? []).map((r) => ({
    fiscalDate: r.date,
...
// Daily closes, most-recent-first, invalid rows dropped.
export function normalizePriceHistory(
  raw: FmpHistoricalPrices,
): NormalizedPricePoint[] {
  return (raw?.historical ?? [])
    .filter((p) => p.date && typeof p.close === "number" && p.close > 0)
