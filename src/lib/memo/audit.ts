...
// never from the LLM" contract (must not be weakened; see product spec). It
// scans generated prose for numeric claims and flags any figure that doesn't
// trace back to the Financial Packet, so the UI can mark unverified numbers
...
// Numbers we never flag: years, small list/section integers.
...
export function auditText(text: string, known: Set<string>): AuditResult {
  ...
  for (const raw of matches) {
    ...
    const num = parseFloat(
      token.replace(/[$,×x%\s]/gi, "").replace(/[TBMK]/i, ""),
    );
