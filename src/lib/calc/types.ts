...
// the Company UI and the LLM memo pipeline consume.
//
// Critical design rule: every quantitative value carries a ReliabilityFlag and
// (where applicable) provenance. Numbers never appear without provenance, and
// the LLM is only ever given this packet — it does not invent figures.

import type { ReliabilityFlag } from "@/lib/enums";
import type { Discrepancy } from "@/lib/data/reconcile";
import type { ReverseDcfResult } from "@/lib/calc/reverse-dcf";
...
// One historical period's computed view (used for trend tables).
...
export interface PacketMetrics {
  ...
  // Risk (from daily price history)
  realizedVol1yPct: MetricValue;
}
...
export interface PacketReverseDcf {
...
export interface FinancialPacket {
  ...
  /** cross-provider disagreements >2% — stored and surfaced, never discarded */
