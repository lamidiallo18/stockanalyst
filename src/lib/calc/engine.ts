...
//
// Takes normalized provider data and produces a FinancialPacket. Contains NO
// I/O and NO randomness, so it is fully unit-testable and reproducible. Every
// output value is tagged with a ReliabilityFlag describing how trustworthy it
// is (REPORTED / DERIVED / ESTIMATE / PROXY / STALE / MISSING).
// ===========================================================================
...
export interface CalcInput {
  ...
  quote?: NormalizedQuote;
  financials: NormalizedFinancials; // annual periods
...
function mv(
  ...
  opts: { unit?: string; note?: string; sourceKey?: string } = {},
): MetricValue {
  return {
...
/** Enterprise value: prefer reported; else marketCap + debt - cash (proxy). */
function enterpriseValue(
  ...
  if (
    ...
    const ev =
      quote.marketCap + (latest.totalDebt ?? 0) - (latest.cash ?? 0);
...
function buildPeriodView(p: NormalizedPeriod): PacketPeriod {
  ...
  return {
    ...
    grossMargin: mv(marginPct(p.grossProfit, p.revenue), DERIVED, {
      unit: "%",
    }),
...
function computePeer(input: CalcPeerInput): PacketPeer {
  ...
  return {
    ...
    revenueCagr3y: mv(
      cagrPct(oldest3?.revenue, latest?.revenue, 3),
      DERIVED,
      { unit: "%" },
    ),
  };
...
export function computePacket(input: CalcInput): FinancialPacket {
  ...
  // --- Margins (latest FY) ---
  const latestFcf = latest ? periodFcf(latest) : { value: null, reliability: MISSING };

  // --- ROIC ---
  const roic = ((): MetricValue => {
    if (!latest || !isNum(latest.opIncome)) return missing("%", "Operating income unavailable.");
    const invested =
      (latest.totalDebt ?? null) !== null &&
      (latest.totalEquity ?? null) !== null
    ...
    if (isNum(latest.pretaxIncome) && latest.pretaxIncome > 0 && isNum(latest.incomeTax)) {
  ...
  // --- Valuation (current) ---
  const pe = mv(safeDiv(marketCap, latest?.netIncome), quoteReliability === STALE ? STALE : DERIVED, { unit: "x" });
  const evEbitda = mv(safeDiv(ev.value, latest?.ebitda), ev.reliability === MISSING ? MISSING : DERIVED, { unit: "x" });
  const evSales = mv(safeDiv(ev.value, latest?.revenue), ev.reliability === MISSING ? MISSING : DERIVED, { unit: "x" });
  ...
  const metrics = {
    ...
    grossMargin: latest ? mv(marginPct(latest.grossProfit, latest.revenue), DERIVED, { unit: "%" }) : missing("%"),
    opMargin: latest ? mv(marginPct(latest.opIncome, latest.revenue), DERIVED, { unit: "%" }) : missing("%"),
    ebitdaMargin: latest ? mv(marginPct(latest.ebitda, latest.revenue), DERIVED, { unit: "%" }) : missing("%"),
    netMargin: latest ? mv(marginPct(latest.netIncome, latest.revenue), DERIVED, { unit: "%" }) : missing("%"),
    fcfMargin: latest ? mv(marginPct(latestFcf.value, latest.revenue), DERIVED, { unit: "%" }) : missing("%"),
    fcf: mv(latestFcf.value, latestFcf.reliability, { unit: "$" }),
    capexToRevenue: latest ? mv(
      latest.capex !== undefined ? marginPct(Math.abs(latest.capex), latest.revenue) : null,
      DERIVED, { unit: "%" },
    ) : missing("%"),
    roic,
    roe,
  ...
  const tracked: MetricValue[] = [
    ...
    metrics.roic, metrics.roe, metrics.netDebtToEbitda,
  ...
  // Record which headline metrics are missing for transparency.
  ...
  completeness.missing = labelMap.filter(([m]) => m.value === null).map(([, n]) => n);
  ...
  return {
    ...
    profile: {
      ...
      sourceKey: input.profile.cik ? "edgar" : undefined,
    },
    quote: {
