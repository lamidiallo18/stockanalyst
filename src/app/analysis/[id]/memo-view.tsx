...
export function MemoView({ id }: { id: string }) {
  ...
  if (!data.memo)
    return (
      <Card>
        <p className="text-sm">
          No memo generated yet (status: {data.analysis.status}).
        </p>
      </Card>
    );
      ...
      <Card>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            {m.rating && (
              <Badge tone={ratingTone[m.rating] ?? "default"}>{m.rating}</Badge>
            )}
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
          <Stat
            label="Suggested size (computed)"
            value={
              m.positionSizeLow != null
                ? `${m.positionSizeLow}–${m.positionSizeHigh}%`
                : "—"
            }
          />
          <Stat
            label="Composite"
            value={m.compositeScore != null ? `${m.compositeScore}/100` : "—"}
          />
          <Stat label="Horizon" value={m.timeHorizon ?? "—"} />
          <Stat
            label="Model"
            value={`${m.providerKey ?? "?"} / ${m.modelUsed ?? "?"}`}
          />
        </div>
        {m.sizingRationale && (
          <p className="mt-2 text-xs text-[var(--muted)]">
            Sizing: {m.sizingRationale}
          </p>
        )}
        <div className="mt-2 flex flex-wrap gap-3 text-xs text-[var(--muted)]">
          <span>
            Cost: est {formatUsd(m.estimatedCostUsd)} → actual{" "}
            {formatUsd(m.actualCostUsd)}
          </span>
          <span>
        ...
        {m.providerKey === "mock" && (
          <p className="mt-3 rounded-md bg-[var(--warning)]/15 px-3 py-2 text-xs text-[var(--warning)]">
            Generated with the Mock provider — narrative is placeholder. The
            numbers, scores, implied growth and sizing are real. Connect
            Anthropic/OpenAI in Settings for genuine analysis.
          </p>
        )}
        {m.totalUnverified > 0 && (
          <p className="mt-2 text-xs text-[var(--warning)]">
            ⚠ {m.totalUnverified} figure(s) across the memo could not be traced
            to the data packet — see flags below.
          </p>
        )}
      </Card>
          ...
          {m.scores.map((s) => (
            <div key={s.category} className="rounded-lg border p-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {s.clamped && <Badge tone="warning">clamped</Badge>}
                  {s.comparative && <Badge tone="accent">vs peers</Badge>}
                  {s.needsQualitative && <Badge>qualitative</Badge>}
                  <Badge
                    tone={
                      s.dataCompleteness === "HIGH"
                        ? "positive"
                        : s.dataCompleteness === "MEDIUM"
                          ? "warning"
                          : "negative"
                    }
                  >
